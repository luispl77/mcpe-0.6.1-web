 ...
 * Per-player saves
 *
 * A file each, under <level>/players/, in the same three-part shape as
 * level.dat -- version, size, NBT -- so that anything already able to read one
 * of these files can read the other, and an operator can see at a glance whose
 * is whose.
 *
 * Deliberately not inside level.dat. LevelData has room for exactly one player
 * and writes players[0]; widening that would change the format every existing
 * world is written in, to solve a problem only a dedicated world has. A
 * directory alongside it costs nothing and a world without one simply has
 * nobody saved yet, which is exactly what every world made before today is.
 * ------------------------------------------------------------------------- */

const char* const fnPlayersDir = "players";

std::string ExternalFileLevelStorage::playerFileName(const std::string& name)
{
	/* A name is chosen by whoever is joining, so it reaches here as somebody
	   else's text and must not be allowed to be a path. Everything outside
	   [a-z0-9_-] becomes an underscore -- no dots, so no "..", and no slashes,
	   so no climbing out of the level directory.

	   Two names that differ only outside that set collapse onto one file and
	   would share a position. That is a real if unlikely collision, and the
	   alternative -- hashing -- costs an operator the ability to see whose file
	   is whose, which is worth more here than the last edge of correctness. */
	std::string out;
	for (unsigned int i = 0; i < name.size() && out.size() < 32; ++i) {
		const char c = name[i];
	...
	// All underscores is not a name; nor is nothing.
	for (unsigned int i = 0; i < out.size(); ++i)
		if (out[i] != '_') return out;
	return std::string();
}

bool ExternalFileLevelStorage::loadPlayerData(const std::string& name, Player* player)
{
	const std::string safe = playerFileName(name);
	if (safe.empty() || !player)
		return false;

	const std::string path = levelPath + "/" + fnPlayersDir + "/" + safe + ".dat";
	FILE* file = fopen(path.c_str(), "rb");
	if (!file)
		return false;   // never joined this world before: spawn, as always
	...
	do {
		...
		// Same guard readLevelData uses: a truncated or hand-edited file must
		// be a player who spawns, not an allocation the size of a claim.
		const int left = getRemainingFileSize(file);
		if (size > left || size <= 0) break;

		data = new unsigned char[size];
		if (fread(data, 1, size, file) != (size_t)size) break;

		RakNet::BitStream bitStream(data, size, false);
		RakDataInput stream(bitStream);
		CompoundTag* tag = NbtIo::read(&stream);
		if (tag) {
			loaded = player->load(tag);
			tag->deleteChildren();
			delete tag;
		}
	} while (false);
...
void ExternalFileLevelStorage::savePlayerData(const std::string& name, Player* player)
{
	const std::string safe = playerFileName(name);
	if (safe.empty() || !player)
		return;

	const std::string dir = levelPath + "/" + fnPlayersDir;
	if (!createFolderIfNotExists(dir.c_str()))
		return;
	...
	/* Through a temporary and renamed into place, the way level.dat is written.
	   A player file is small enough that a torn write is unlikely and bad
	   enough that it is not worth risking: half a position is a player who
	   spawns inside the ground. */
	const std::string tmp = dir + "/" + safe + ".dat_new";
	const std::string dat = dir + "/" + safe + ".dat";

	FILE* file = fopen(tmp.c_str(), "wb");
	if (!file)
		return;

	const int version = SharedConstants::StorageVersion;
	const int size = data.GetNumberOfBytesUsed();
	...
	remove(dat.c_str());
	if (rename(tmp.c_str(), dat.c_str()))
		LOGE("Error@savePlayerData: couldn't move new file into place for %s\n", safe.c_str());
}
